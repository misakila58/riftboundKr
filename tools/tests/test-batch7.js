...
//  ⑬ 쌍권총 난사(268) — 힘 0 지불이면 피해 0 인스턴스라 관문 +1 없음(2026-07-16판 717.3, 구판 #5791 뒤집음)
...
(async()=>{
  ...
  await play(268);
  // 2026-07-16판 717.3 'If no damage was Dealt, then Bonus Damage will not apply' — 힘 0 = 피해 0 인스턴스라 관문·서적 보너스 없음 (구판 #5791과 반대)
  ok('쌍권총 난사: 힘 0이면 피해 액션이 없어 관문의 +1도 없음(717.3)', vg.dmg===0, 'dmg='+vg.dmg);
  fresh([296,297]); vg=unit(219,1,0); OPT=(t,o)=>0; NUM=()=>0; TF().nextSpellBonus[0]=1;
  await play(268);
  ok('쌍권총 난사: 힘 0 + 관문 + 서적 = 0(717.3)', vg.dmg===0, 'dmg='+vg.dmg);
