...
- **룰 절충(의도된 단순화 — 룰북 대조 감사 2026-08 완료, 전 카드 원문 대조 전수 감사 2026-08-27 완료·47건 수정)**: ① 격발(트리거) 능력은 체인에 쌓이지 않고 즉시 해결(공식은 Initial Chain·응수 가능, 방어자 트리거 선해결) ② 복수 전장 동시 경합 시 인덱스 순 고정(공식은 턴 플레이어 선택) ③ [탱커]를 부여받은 '마지막 배분' 유닛(케이틀린)은 항상 마지막 배분(공식은 선택 허용) ④ 주문의 '이동 목적지'(352.7)와 다중 대상(각 N개까지·분할 피해)은 아직 해결 시점 선택. (정식 구현으로 제거된 항목: 카운터의 카운터·중립 [반응] 능력 응수 2026-08-31 → test-chain2.js / **주문 단일 대상은 플레이 시점 지정·해결 시 부적법이면 그 지시만 불발(352.8.a·356.3.e) 2026-09-09 → preTargetSpell·PRE_TARGET_OPS, test-target.js** · 전용 op도 동일(cardscripts `PRE_TARGET_EXTRA` spec 목록 → engine `preTargetSpecs`/`takePreTarget`이 pickBySpec·chooseReturnToHand·chooseEffectMove에서 소비) · **적법 대상 없으면 플레이 불가·굴절 거부 시 재선택/취소(352.8·735) → `spellHasTargets`(playRestriction·polCanPlay 공용)·`pickPreTarget`, 대상 지정은 비용 지불 전(352→353)** · 'Do this N번' 반사 격발 주문(떨어지는 별·이케시아 소나기)은 `fx.reflexive`로 해결 시점 선택(383) → test-batch2.js. 「약자 도태」처럼 '각자 고르는' 지시는 대상 지정이 아니라 해결 시 선택 — RiftJudge #10900) 룰북 원문: `tools/data/rules1.txt` 1107~1610행(상태·우선권·체인·결전), 3290~3490행(전투).
