...
- **룰북 버전**: 최신 공식 종합 규칙은 `tools/data/rules-2026-07-16.txt`(Vendetta판, Rules Hub PDF에서 `pdftotext -layout`). `tools/data/rules1.txt`는 2025-12-01판으로, **엔진 주석·테스트가 인용하는 조항 번호는 이 구판 기준**(대응: 352.5~8→355.9, 356.3.e→359.3.e, 373→362, 381~385→387~390, 437~446→460~476, 453~457→477, 731→805, 735→809, 737~739→811, 군단→812). 공식 에라타(2026-01-14 Spiritforged FAQ)는 `tools/rules-judge/data/errata-spiritforged-2026-01-14.md` — 떨어지는 별·이케시아 소나기는 반사 격발이 아니라 보통 주문(대상 선지정, 굴절은 고를 때마다 809). 룰 패치(Unleashed 2026-03-31·Vendetta 2026-07-16)의 Origins 영향 요약은 `tools/data/patchnotes-unleashed-vendetta-요약.md`.
- **새 룰북 대조 감사 (2026-09-11)**: 2026-07-16판 전수 대조 → 발견 52건 → 수정 23건(결전 승격·통제 상실 시점·전투 해결 순서·플레이 수 파이널라이즈 기준(playedCards)/격발 순번(playedSeq) 분리·활성화 능력 대상 선지정·할인 최소값·번아웃 반복 등). 테스트 `tools/tests/test-rules2026-a.js`(25)·`test-rules2026-b.js`(35). 내역 `docs/룰대조-변경내역-2026-09-11-신룰북.md`, 감사 도구 `tools/rules-audit-merge.js`(입력 `tools/data/rules-audit-2026/`, gitignore).
...
- 카드 데이터 수정: `tools/data/`의 번역본 수정 → `node tools/build-cards.js` (요청 시에만). **주의(2026-09-14 확인)**: 커밋된 `cards.js`/`server/cards.json`은 tools/data보다 앞서 있다(이미지 URL `?accountingTag=RB`, 일부 전설 super:'Champion' 등) — 지금 재생성하면 그 차이가 되돌아가므로, 필드 하나를 더하는 정도면 재생성 대신 기존 파일을 스크립트로 패치할 것. `server/cards.json`은 시그니처·전설·챔피언 유닛만 tags를 가진다(선발 챔피언–전설 태그 검사, 룰 103).
...
- **룰 절충(의도된 단순화 — 룰북 대조 감사 2026-08 완료, 전 카드 원문 대조 전수 감사 2026-08-27 완료·47건 수정)**: ① 격발(트리거) 능력은 체인에 쌓이지 않고 즉시 해결(공식은 Initial Chain·응수 가능, 방어자 트리거 선해결) ② 복수 전장 동시 경합 시 인덱스 순 고정(공식은 턴 플레이어 선택) ③ (삭제 — Unleashed Backline 키워드로 '마지막 배분'이 공식이 되어 일치) ④ 주문의 '이동 목적지'(352.7)와 다중 대상(각 N개까지·분할 피해)은 아직 해결 시점 선택. (정식 구현으로 제거된 항목: 카운터의 카운터·중립 [반응] 능력 응수 2026-08-31 → test-chain2.js / **주문 단일 대상은 플레이 시점 지정·해결 시 부적법이면 그 지시만 불발(352.8.a·356.3.e) 2026-09-09 → preTargetSpell·PRE_TARGET_OPS, test-target.js** · 전용 op도 동일(cardscripts `PRE_TARGET_EXTRA` spec 목록 → engine `preTargetSpecs`/`takePreTarget`이 pickBySpec·chooseReturnToHand·chooseEffectMove에서 소비) · **적법 대상 없으면 플레이 불가·굴절 거부 시 재선택/취소(352.8·735) → `spellHasTargets`(playRestriction·polCanPlay 공용)·`pickPreTarget`, 대상 지정은 비용 지불 전(352→353)** · 떨어지는 별·이케시아 소나기는 공식 에라타(2026-01-14)로 **보통 주문**(낼 때 대상 전부 지정·같은 유닛 반복 가능·굴절은 고를 때마다) — `fx.reflexive`는 진짜 'Then do this:' 카드용으로만 남김(Origins에는 없음). 「약자 도태」처럼 '각자 고르는' 지시는 대상 지정이 아니라 해결 시 선택 — RiftJudge #10900) 룰북 원문: `tools/data/rules1.txt` 1107~1610행(상태·우선권·체인·결전), 3290~3490행(전투).
...
- **봇 개선용 리플레이 수집 (2026-09-16)**: 경기 종료 시 클라이언트(`replay.js` REPLAY.share, 기본 켜짐·설정 토글 `rb_replay_share_off`, 닉네임 익명화 rpAnonymize)가 `.rbr`를 서버 `POST /api/replay`로 보낸다 → 서버는 `data/replays/<YYYY-MM>/*.rbr` + `index.jsonl`에 보관하고 봇전 집계(`bot:<level>:games/humanwin/turnsum`, `botdeck:<level>:<legend-champ>:games/wins`)를 STATS에 올린다. `/api/status`의 `REPLAYS|n개` 줄로 개수 확인. 상한: 파일 1.5MB·2만 개·1GB, IP당 시간당 40건. **내려받기**: `scp -r -i ~/.ssh/riftbound root@riftboundsimkr.duckdns.org:/opt/riftbound/data/replays tools/data/replays` (gitignore). 분석: `node tools/replay-extract.js <file.rbr>`(로그→행동 복원) · `node tools/replay-analyze.js tools/data/replays --losses 5 --dataset out.jsonl`(난이도별 전적·덱별 승률·초반 플레이·봇 패배 판 요약·학습용 (상태,행동) 쌍). 첫 실행 안내창은 main.js showFirstRunNotice(`rb_notice_v1`).
