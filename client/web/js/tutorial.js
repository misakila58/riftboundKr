// ══════════ 튜토리얼 모드 ══════════
// 고정 덱 + 스크립트 봇을 상대로 게임 흐름 전체(턴 구조·자원·이동·결전·전투·득점·키워드)를
...
// ---------- 교육용 카드 (다리우스: 분노/질서) ----------
// 210 대담한 포로(2, 2⚔, 맹공)  216 비상하는 정찰병(2, 1⚔, 죽음의 종소리)
// 217 삼두정 영광 추구자(2, 2⚔, 군단-버프)  9 마법공학 광선(1+힘1, 피해3)
// 218 선봉대 대장(3+힘1, 군단-신병토큰2)  4 가르기(1, 맹공3 부여)  240 세트-우두머리(4+힘1, 탱커)
const TUT_P0 = {
...
const TUT_BFS = [280, 297];                     // 신버들 숲(유지:드로우) / 바람쓸린 언덕(개입)
...
function tutSteps(){
  ...
  { kind:'info', title:'🔄 턴 구조 (A-B-C-D)',
    ...
    <b>B 시작</b> — 시작 효과 발동 + <b>유지 득점</b>: <b>유닛이 주둔한</b> 통제 전장마다 1점!
    (유닛이 없으면 무주공산이 되어 통제가 풀립니다)<br>
    <b>C 전개</b> — 룬 덱에서 룬 2개를 가져옵니다 (후공은 첫 턴만 3개).<br>
    <b>D 드로우</b> — 카드 1장을 뽑습니다.<br><br>
    지금 당신의 첫 턴 — 룬 2개가 전개되고 1장을 뽑아 손패가 5장입니다.` },
  { kind:'info', title:'💎 자원: 에너지와 힘',
    text:`카드 비용은 <b>에너지</b>(좌상단 숫자)와 <b>힘</b>(색 구슬)로 지불합니다.<br>
    · <b>에너지</b>: 룬 1개를 <b>탈진</b>(눕히기)할 때마다 1씩. 다음 턴에 다시 일어납니다.<br>
    · <b>힘</b>: 룬 1개를 <b>재활용</b>(룬 덱 맨 아래로 반환)해서 그 룬의 색 힘 1을 얻습니다.<br>
  ...
  { kind:'task', title:'🚶 전장으로 이동 → 정복!',
    ...
    상대 유닛이 없어도 전장에 들어가면 <b>결전</b>이 열립니다 — 상대에게 대응 기회를 주는 것이죠.`,
    hint:'[이동] → 유닛 클릭 → 전장 클릭',
  ...
  { kind:'task', title:'⚔️ 결전: 패스하면 진행됩니다',
    ...
    양쪽 다 패스하면 결전이 해결됩니다. 봇은 자동으로 패스하니,
  ...
  { kind:'task', title:'🛡 방어 결전! 패스하면 전투가 벌어집니다',
    text:`봇이 <b>신병 토큰(1⚔)</b>을 당신의 전장에 보냈습니다 — 이번엔 <b>전투가 있는 결전</b>입니다.<br>
    지금 [행동] 주문으로 요격할 수도 있지만, 일단 <b>[패스]</b>해서 전투를 구경해봅시다.<br><br>
    <b>전투 규칙</b>: 양측 유닛의 <b>위력(⚔) 합계</b>만큼 서로에게 <b>동시에</b> 피해를 배분합니다.
  ...
  { kind:'info', title:'💥 전투 결과 — 방어 성공!',
    ...
    유지 수입도 끊깁니다 — 전장은 유닛이 <b>지키고 있어야</b> 내 것입니다.<br><br>
    봇이 턴을 마치면, 당신 턴 시작에 <b>유지 득점</b>이 들어오는 걸 확인하세요.` },
  { kind:'info', title:'🏆 유지(Hold) 득점!',
  ...
  { kind:'task', title:'▶ 「비상하는 정찰병」 플레이 — [죽음의 종소리]',
    ...
    hint:'손패의 비상하는 정찰병 → ▶ 플레이 → 선택 창에서 [기지]',
  ...
  { kind:'task', title:'▶ 「삼두정 영광 추구자」 — [군단] + 버프',
    ...
    done:()=>everyUnit().some(u=>u.ctrl===0&&u.n===217)||G.players[0].trash.includes(217) },
  ...
  { kind:'task', title:'👑 챔피언 플레이!',
    text:`당신 턴입니다 (유지 +1점!). 이제 <b>챔피언 존</b>의 <b>「다리우스 - 삼두정」</b>
  ...
  { kind:'info', title:'🗡 마무리 수업: 공격 준비',
    ...
    · <b>「가르기」</b>(비용 1): 유닛에게 [맹공 3]을 부여하는 <b>[행동]</b> 주문 — <b>결전 중에도</b> 쓸 수 있어
  ...
  { kind:'end', title:'🎉 튜토리얼 완료!',
    ...
    턴 구조(ABCD) · 에너지/힘 · 유닛 소환/이동 · 결전과 전투 · 정복/유지 득점 ·
    키워드([맹공][죽음의 종소리][군단][탱커][개입]...) · 전설/챔피언 · 승리 조건.<br><br>
...
TUT.runBotTurn=async function(){
  ...
  try{
    if(TUT.botTurn===1){
      ...
      // 예전엔 211(2⚔)로 2vs2 상호 전멸을 시켰는데, v1.0.15부터 빈 전장은 개시에 무주공산이
      // 되므로 다음 '유지 득점' 단계의 진행 조건(points>=2)이 영영 충족되지 않았다.
      const atk=[...G.players[1].base].filter(u=>!u.ex).sort((a,b)=>might(a)-might(b))[0];
      if(atk && G.bfs[0].controller===0){
        await moveUnits(1,[atk],0);
        // 결전은 tick의 자동 패스 + 플레이어 패스로 해결됨 — 해결까지 대기
        let guard=0;
