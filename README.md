...
│     ├─ index.html
│     ├─ css/style.css
│     └─ js/                cards, loc, effects, engine, net, p2p, signal, ui, main
...
### 방법 A — 서버 없이 친구와 직접 대전 (가장 간단) ⭐

클라이언트만 있으면 됩니다. 계정도 필요 없고, 닉네임과 덱은 각자 컴퓨터에만 저장됩니다.

1. 두 사람 모두 **클라이언트**(`RiftboundSim-Setup.exe` 또는 portable)를 실행
2. 첫 화면에서 **[👥 친구와 직접 대전 (서버 없이 · 초대 코드)]** 선택
3. 방장: **[방 코드 만들기]** → 화면에 뜬 **6글자 코드**(예: `K7M-2Q9`)를 카톡/디스코드로 전송
4. 상대: 그 코드를 입력하고 **[참여하기]** → 자동으로 게임 시작!

> WebRTC 직접 연결(P2P) 방식입니다. 방 코드는 두 사람이 서로를 찾는 동안에만 쓰이고,
> 게임 데이터는 두 컴퓨터가 직접 주고받습니다. 코드로 유도한 키로 암호화되어 오갑니다.
>
> 대부분의 가정 환경에서 동작하지만, 일부 사내망/특수 공유기에서는 연결이 안 될 수 있습니다.
> 그럴 때는 화면 아래 **⚙ 고급**에서 두 가지 대안을 쓸 수 있습니다.
> - **초대 코드 직접 교환** — 서버를 전혀 거치지 않고 코드(100자 내외)를 손으로 주고받는 방식
> - **시그널링 서버 지정** — 직접 띄운 서버를 쓰는 방식 (`server/signal-worker/README.md`, 무료·10분)
>
> 그래도 안 되면 방법 B를 쓰세요.

### 방법 B — 상시 서버 운영 (로비/계정/랭킹형 운영에 적합)
...
게임이 얼마나 플레이되는지와 덱 조합별 승률을 파악하기 위해 **익명 집계 숫자**를 보냅니다.
첫 화면의 **📊 익명 통계** 버튼으로 언제든 끌 수 있습니다.

보내는 것

- 모드별(봇전·핫시트·친구 대전·서버 대전) 게임 시작/종료, 턴 수, 중도 이탈 여부
- 덱 조합 — **전설 번호 + 선발 챔피언 번호**와 승패 (공개 카드 정보)
- 앱을 켠 날(하루 1회), 앱 버전

보내지 않는 것

- 닉네임·계정·덱 이름·덱 전체 목록·채팅·리플레이
- 설치 ID·기기 ID — 애초에 만들지 않습니다
- 정확한 시각 (서버가 날짜 단위로만 집계)

수집 서버도 **IP를 기록하지 않고 개별 요청을 저장하지 않습니다** — 도착 즉시 카운터에 +1 하고 버립니다.
따라서 남는 것은 `봇전 12판 시작`, `카이사/다리우스가 애니/고집쟁이에게 7승 3패` 같은 집계 숫자뿐이며,
특정 개인을 알아볼 수 있는 정보가 남지 않습니다.

집계 서버를 배포하지 않으면(`client/web/js/stats.js` 의 `URL` 이 비어 있으면) 기능 전체가 꺼지고
아무 요청도 나가지 않습니다. 자세한 내용은 `server/stats-worker/README.md` 참고.
